...
const doctorList = async (req, res) => {
  try {
    ...
    if (Array.isArray(doctors)) {
      ...
      res.status(200).json({ success: true, data: doctors.reverse() });
    } else {
...
const loginD = async (req, res) => {
  try {
    ...
    if (isINWINKWINK) {
      ...
      const token = jwt.sign({ id: doctor._id }, process.env.JWT_SEC, { expiresIn: '1h' });
...
const getAsA = async (req,res)=>{
  try {
    const Dcont = (await contactFormModel.find({})).filter(item => item.role === 'assistant');
    if(Dcont){
...
const deleteOneAsA = async (req, res) => {
  try {
    const { id } = req.body; 
    const result = await contactFormModel.deleteOne({ _id: id, role: 'assistant' });
