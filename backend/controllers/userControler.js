...
import contactFormSchema from '../model/contactFormModel.js';
...
const formRegi = async (req, res) => {
  try {
    ...
    const D= await contactFormSchema.findOne({email})
    if (D){
      return res.json({ success:false, message: 'pre registered email ' });
}
    if (!name || !email || !phone || !role) {
      return res.status(400).json({ error: 'Required fields are missing.' });
    }

    if (role === 'doctor' && (!expertise || !education  === undefined)) {
      return res
        .json({ success:false, message: 'Expertise, education, and years of expertise are required for doctors.' });
    }

    const newContactForm = new contactFormSchema({
      name,
      email,
      phone,
      role,
      expertise: role === 'doctor' ? expertise : undefined,
...
export { registerUser,getProfile,cancelapp,sendAppList,formRegi, updateUserP,loginUser ,bookApp};
